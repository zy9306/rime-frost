...
local function yield_cand(seg, text, quality)
...
function M.init(env)
    ...
    M.quality = config:get_int(env.name_space .. '/quality') or 100
end
...
function M.func(input, seg, env)
    ...
    if (input == M.date) then
        ...
        yield_cand(seg, os.date('%Y-%m-%d', current_time), M.quality)
        yield_cand(seg, os.date('%Y/%m/%d', current_time), M.quality)
        yield_cand(seg, os.date('%Y.%m.%d', current_time), M.quality)
        yield_cand(seg, os.date('%Y%m%d', current_time), M.quality)
        yield_cand(seg, os.date('%Y年%m月%d日', current_time):gsub('年0', '年'):gsub('月0','月'), M.quality)

    -- 时间
    elseif (input == M.time) then
        local current_time = os.time()
        yield_cand(seg, os.date('%H:%M', current_time), M.quality)
        yield_cand(seg, os.date('%H:%M:%S', current_time), M.quality)
    ...
    elseif (input == M.week) then
        ...
        yield_cand(seg, '星期' .. text, M.quality)
        yield_cand(seg, '礼拜' .. text, M.quality)
        yield_cand(seg, '周' .. text, M.quality)

    -- ISO 8601/RFC 3339 的时间格式 （固定东八区）（示例 2022-01-07T20:42:51+08:00）
    elseif (input == M.datetime) then
        local current_time = os.time()
        yield_cand(seg, os.date('%Y-%m-%dT%H:%M:%S+08:00', current_time), M.quality)
        yield_cand(seg, os.date('%Y-%m-%d %H:%M:%S', current_time), M.quality)
        yield_cand(seg, os.date('%Y%m%d%H%M%S', current_time), M.quality)

    -- 时间戳（十位数，到秒，示例 1650861664）
    elseif (input == M.timestamp) then
        local current_time = os.time()
        yield_cand(seg, string.format('%d', current_time), M.quality)
    end
